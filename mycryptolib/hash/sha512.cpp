...
void MyCryptoLib::SHA512::update(const std::string &data)
{
    this->update(std::vector<uint8_t>(data.begin(), data.end()));
}

void MyCryptoLib::SHA512::update(const std::vector<uint8_t> &data)
{
...
void MyCryptoLib::SHA512::update(std::ifstream &file, size_t bytesCount)
{
    if (!file.is_open())
    {
...
size_t MyCryptoLib::SHA512::blockSize()
{
    return 128;
}

void MyCryptoLib::SHA512::_updateState(const std::vector<uint8_t> &buffer)
{
    int pos = 0;
...
void MyCryptoLib::SHA512::_processBlock(std::vector<uint64_t> block)
{
    for (int i = 16; i < 80; i++)
    {
        block[i] = block[i - 16] +
...
void MyCryptoLib::SHA512::_pad(std::vector<uint8_t> &buffer, size_t dataSize)
{
...
void MyCryptoLib::SHA512::_finalize()
{
    for (int i = 0; i < 8; i++)
    {
...
uint64_t MyCryptoLib::SHA512::Sigma0(uint64_t x)
{
    return SHA512::rotr(x, 1) ^ SHA512::rotr(x, 8) ^ (x >> 7);
}

uint64_t MyCryptoLib::SHA512::Sigma1(uint64_t x)
{
...
uint64_t MyCryptoLib::SHA512::rotr(uint64_t value, size_t bits)
{
    return (value >> bits) | (value << ((sizeof(uint64_t) * 8) - bits));
}

uint64_t MyCryptoLib::SHA512::Ch(uint64_t x, uint64_t y, uint64_t z)
{
    return (x & y) ^ (~x & z);
}

uint64_t MyCryptoLib::SHA512::Maj(uint64_t x, uint64_t y, uint64_t z)
{
    return (x & y) ^ (x & z) ^ (y & z);
}
