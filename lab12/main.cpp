...
void user_1()
{
    ...
    uint8_t myRandom[4] = {0x00};
    std::vector<uint8_t> buffer(8, 0x00);
    std::vector<uint8_t> encBuffer;
    std::vector<uint8_t> completeBuffer;

    srand(time(nullptr));
    buffer[0] = 0x01;   // Представим что это М
    buffer[1] = 0x0A;
    buffer[2] = 0xFF;
    buffer[3] = 0x00;

    buffer[4] = myRandom[0] = rand() & 0xFF; // Представим что это рандом нумбер
    buffer[5] = myRandom[1] = rand() & 0xFF;
    buffer[6] = myRandom[2] = rand() & 0xFF;
    buffer[7] = myRandom[3] = rand() & 0xFF;
    ...
    if (buffer[0] != myRandom[0] || buffer[1] != myRandom[1] ||
            buffer[2] != myRandom[2] || buffer[3] != myRandom[3])
    {
        std::cerr << "RANDOM VERIFY ERROR" << std::endl;
        close(opSocket);
        close(mySocket);
        return;
    }

    std::vector<uint8_t> opId(buffer.begin() + 8, buffer.end() - USER_ID_TERMINATOR.size()); // я не знаю зачем дописывать сообщение после ID
    ...
    buffer.resize(USER_1_ID.size() + 8 + USER_ID_TERMINATOR.size());

    std::swap(buffer[0], buffer[4]);
    std::swap(buffer[1], buffer[5]);
    std::swap(buffer[2], buffer[6]);
    std::swap(buffer[3], buffer[7]);
...
void user_2()
{
    ...
    srand(time(nullptr));
    myRandom[0] = rand() & 0xFF;
    myRandom[1] = rand() & 0xFF;
    myRandom[2] = rand() & 0xFF;
    myRandom[3] = rand() & 0xFF;
    ...
    buffer.resize(8 + USER_2_ID.size() + USER_ID_TERMINATOR.size());

    std::copy(completeBuffer.begin() + 4, completeBuffer.end(), buffer.begin());
    std::copy(myRandom, myRandom + 4, buffer.begin() + 4);

    std::copy(USER_2_ID.begin(), USER_2_ID.end(), buffer.begin() + 8);
    std::copy(USER_ID_TERMINATOR.begin(), USER_ID_TERMINATOR.end(), buffer.end() - USER_ID_TERMINATOR.size());
    ...
    if (!(completeBuffer[0] == 0x01 &&
          ...
          completeBuffer[3] == 0x00))
    {
        std::cerr << "BAD MESSAGE HEADER RECVED" << std::endl;
        close(sockfd);
        return;
    }
    ...
    if (buffer[0] != myRandom[0] || buffer[1] != myRandom[1] || buffer[2] != myRandom[2] || buffer[3] != myRandom[3])
    {
        std::cerr << "RANDOM VERIFY ERROR" << std::endl;
        close(sockfd);
        return;
