...
D35Crypto::PKCS7::PKCS7(const std::string &filename):
...
D35Crypto::PKCS7::PKCS7(const std::map<int, std::vector<uint8_t> > &data) :
    PKCSBase("-----BEGIN PKCS7 ENCRYPTED DATA-----", "-----END PKCS7 ENCRYPTED DATA-----")
{
    if (data.size() > 0)
    {
...
D35Crypto::PKCS7 D35Crypto::PKCS7::fromFile(const std::string &filename)
{
    std::ifstream pkcsFile(filename);
    if (!pkcsFile.is_open())
    {
...
//D35Crypto::PKCS7Data D35Crypto::PKCS7Data::fromFile(const std::string &filename)
//{
...
//        if (marker != pemHeader)
//        {
//            throw D35Crypto::BadPKCSFileStructureException("File: " + filename + " is corrupted");
//        }
//    }
...
D35Crypto::PKCS7 D35Crypto::PKCS7::create(uint8_t version, const std::string &contentType, const std::string &encryptAlgorithmId, const std::vector<uint8_t> &encryptedData)
{
    std::map<int, std::vector<uint8_t>> dataMap;
...
std::vector<uint8_t> D35Crypto::PKCS7::getData() const
{
