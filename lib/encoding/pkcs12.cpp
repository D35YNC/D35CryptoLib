...
D35Crypto::PKCS12::PKCS12(const std::string &keyAlgorithm, const std::map<int, std::vector<uint8_t> > &data) :
    PKCSBase("-----BEGIN " + keyAlgorithm + " PRIVATE KEY-----", "-----END " + keyAlgorithm + " PRIVATE KEY-----"), keyAlgorithm(keyAlgorithm)
{
    this->pkcsData = data;
}

D35Crypto::PKCS12::PKCS12(const std::string &filename) : PKCSBase("", "")
{
