...
D35Crypto::CAdES::CAdES(const std::map<int, std::vector<uint8_t>> &signData):
...
D35Crypto::CAdES D35Crypto::CAdES::create(uint8_t version, const std::string &contentType, const std::string &signerId, const std::vector<uint8_t> &signerPubKeyHash,
...
D35Crypto::CAdES D35Crypto::CAdES::fromBytes(const std::vector<uint8_t> &buffer)
{
    ...
    if (userSignHeaderPos == std::string::npos)
    {
        throw D35Crypto::BadPKCSFileStructureException("Cant find pkcs sign header");
    }
    if (userSignTerminatorPos == std::string::npos)
    {
        throw D35Crypto::BadPKCSFileStructureException("Cant find pkcs sign terminator");
    }
    if (userSignHeaderPos > userSignTerminatorPos)
    {
        throw D35Crypto::BadPKCSFileStructureException("Confusing pkcs structure, cant read");
    }
    ...
    D35Crypto::CAdES cades = D35Crypto::CAdES::parseBuffers(userSignBuffer, caSignBuffer);
...
D35Crypto::CAdES D35Crypto::CAdES::parseBuffers(const std::vector<uint8_t> &userSign, const std::vector<uint8_t> &caSign)
{
    size_t pos = 0;
    size_t fieldSize = 0;
    ...
    CAdES cades = D35Crypto::CAdES(signMap);
...
D35Crypto::CAdES D35Crypto::CAdES::fromFile(const std::string &filename)
{
    ...
    return D35Crypto::CAdES::parseBuffers(fileBuffer);
}


void D35Crypto::CAdES::appendCASign(uint64_t time, const std::vector<uint8_t> &pubKeyHash, const std::vector<uint8_t> &signedMessageDigest, const std::vector<uint8_t> &signature)
{
...
bool D35Crypto::CAdES::isSignedByCA() const
{
    return !this->caSignData.empty();
}

uint8_t D35Crypto::CAdES::getVersion() const
{
    return this->signData.at(0)[0];
}

std::string D35Crypto::CAdES::getContentType() const
{
    std::vector<uint8_t> buf = this->signData.at(1);
    return std::string(buf.begin(), buf.end());
}

std::vector<uint8_t> D35Crypto::CAdES::getContentHash() const
{
    return this->signData.at(2);
}

std::string D35Crypto::CAdES::getSignerName() const
{
    std::vector<uint8_t> buf = this->signData.at(3);
    return std::string(buf.begin(), buf.end());
}

std::vector<uint8_t> D35Crypto::CAdES::getSignerKeyFingerprint() const
{
    return this->signData.at(4);
}

std::string D35Crypto::CAdES::getHashAlgorithmId() const
{
    std::vector<uint8_t> buf = this->signData.at(5);
    return std::string(buf.begin(), buf.end());
}

std::string D35Crypto::CAdES::getSignAlgorithmId() const
{
...
std::vector<uint8_t> D35Crypto::CAdES::getSignature() const
{
    return this->signData.at(7);
}

uint64_t D35Crypto::CAdES::getCATimestamp() const
{
...
std::vector<uint8_t> D35Crypto::CAdES::getCAKeyFingerprint() const
{
    if (!this->isSignedByCA())
    {
        throw std::runtime_error("is not signed");
...
std::vector<uint8_t> D35Crypto::CAdES::getCASignedMessageDigest() const
{
    if (!this->isSignedByCA())
    {
        throw std::runtime_error("is not signed");
...
std::vector<uint8_t> D35Crypto::CAdES::getCASignature() const
{
    if (!this->isSignedByCA())
    {
        throw std::runtime_error("is not signed");
...
size_t D35Crypto::CAdES::getUserSignHeaderPos() const
{
    return this->userSignHeaderPos;
}

size_t D35Crypto::CAdES::getUserSignTerminatorPos() const
{
     return this->userSignTerminatorPos;
}

size_t D35Crypto::CAdES::getCASignHeaderPos() const
{
    return this->caSignHeaderPos;
}

size_t D35Crypto::CAdES::getCASignTerminatorPos() const
{
    return this->caSignTerminatorPos;
}

std::vector<uint8_t> D35Crypto::CAdES::toBytes() const
{
    size_t signSize = 0;
...
std::vector<uint8_t> D35Crypto::CAdES::toPem() const
{
    std::stringstream ss;
    std::string b64string = Base64::encode(this->toBytes());
