...
void D35Crypto::SHA256::update(const std::string &data)
{
    this->update(std::vector<uint8_t>(data.begin(), data.end()));
}

void D35Crypto::SHA256::update(const std::vector<uint8_t> &data)
{
...
void D35Crypto::SHA256::update(std::ifstream &file)
{
    if (!file.is_open())
    {
...
void D35Crypto::SHA256::_processBlock(std::vector<uint32_t> block)
{
    for (int i = 16; i < 64; i++)
    {
        block[i] = block[i - 16] +
...
void D35Crypto::SHA256::_pad(std::vector<uint8_t> &buffer, size_t datasize)
{
...
uint32_t D35Crypto::SHA256::Sigma0(uint32_t x)
{
    return SHA256::rotr(x, 7) ^ SHA256::rotr(x, 18) ^ (x >> 3);
}

uint32_t D35Crypto::SHA256::Sigma1(uint32_t x)
{
...
uint32_t D35Crypto::SHA256::rotr(uint32_t value, size_t bits)
{
    return (value >> bits) | (value << ((sizeof(uint32_t) * 8) - bits));
}

uint32_t D35Crypto::SHA256::Ch(uint32_t x, uint32_t y, uint32_t z)
{
    return (x & y) ^ (~x & z);
}

uint32_t D35Crypto::SHA256::Maj(uint32_t x, uint32_t y, uint32_t z)
{
    return (x & y) ^ (x & z) ^ (y & z);
}
