...
D35Crypto::PKCS7 D35Crypto::RSA::encrypt(const std::string &dataType, const std::vector<uint8_t> &data, const RSAKey &key)
{
    return D35Crypto::PKCS7::create(0, dataType, "RSA-RAW", this->encrypt(data, key));
}


std::vector<uint8_t> D35Crypto::RSA::decrypt(const PKCS7 &pkcs7obj, const RSAKey &key)
{
    return this->decrypt(pkcs7obj.getData(), key);
}


D35Crypto::CAdES D35Crypto::RSA::sign(
        ...
        const RSAKey &key)
{
    return D35Crypto::RSA::sign(username, pubKeyHash, "TEXT", std::vector<uint8_t>(data.begin(), data.end()), hash, key);
}
...
void D35Crypto::RSA::signCA(CAdES &userCAdES, const std::vector<uint8_t> &caPubKeyHash, const std::vector<uint8_t> &signedMessage, const RSAKey &key)
{
    // gEt timestamp
...
bool D35Crypto::RSA::checkSign(const std::vector<uint8_t> &userSignedMessage, const D35Crypto::CAdES &cades, const RSAKey &key)
{
...
bool D35Crypto::RSA::checkCASign(const std::vector<uint8_t> &signedMessage, const CAdES &cades, const RSAKey &caPubKey)
{
